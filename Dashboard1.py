...
for state in data['States']:
    if state["StateName"] in ["Maharashtra", "Madhya Pradesh", "Manipur", "Meghalaya", "Karnataka", "Kerala", "Mizoram",
        ...
        for city in state["Cities"]:
            if "Category" in city and city["Category"] == "Historical":
                city_image = city["CityImage"]
                alternative_images = [image.split(".")[0] for i, image in enumerate(city_image) if i % 2 == 0]
                for cityimage in alternative_images:
                    CITY_IMAGE[city["CityName"]] = cityimage

                BEST_SELLER_TITLE = city["CityName"] + " " + city["IdealLengthOfTrip"]
                BEST_SELLER[BEST_SELLER_TITLE] = cityimage
...
BEST_SELLER_10 = dict(random.sample(list(BEST_SELLER.items()), 1))
SPOT_IMAGE_10 = dict(random.sample(list(SPOT_IMAGE.items()), 10))
CITY_IMAGE_10 = dict(random.sample(list(CITY_IMAGE.items()), 1))
STATE_IMAGE_10 = dict(random.sample(list(STATE_IMAGE.items()), 9))
UT_IMAGE_10 = dict(random.sample(list(UT_IMAGE.items()), 3))

pprint(CITY_IMAGE)
pprint(BEST_SELLER)
pprint(SPOT_IMAGE)
pprint(UT_IMAGE)
pprint(BEST_SELLER_10)
pprint(SPOT_IMAGE_10)
pprint(CITY_IMAGE_10)
pprint(STATE_IMAGE_10)
pprint(UT_IMAGE_10)
